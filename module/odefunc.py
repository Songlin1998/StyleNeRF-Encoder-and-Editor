...
class ODEnet(nn.Module):
    ...
    def __init__(self, hidden_dims, input_shape, context_dim, layer_type="concat", nonlinearity="softplus", extend_dim=0):
        ...
        for dim_out in (hidden_dims + (input_shape[0],)):
            ...
            layer_kwargs = {}
            layer = base_layer(hidden_shape[0], dim_out, context_dim, extend_dim, **layer_kwargs)
            layers.append(layer)
...
class ODEfunc(nn.Module):
    def __init__(self, diffeq, extend_dim):
    ...
    def forward(self, t, states):
        y = states[0]
        t = torch.ones(y.size(0), self.extend_dim).to(y) * t.clone().detach().requires_grad_(True).type_as(y)
        self._num_evals += 1
        for state in states:
        ...
        with torch.set_grad_enabled(True):
            if len(states) == 3:  # conditional CNF
                ...
                c = states[2]
                # print('t:',t.shape) # torch.Size([5, 1])
                # print('c.view(y.size(0)',c.view(y.size(0), -1).shape) # torch.Size([5, 17])
                tc = torch.cat([t, c.view(y.size(0), -1)], dim=1)
                # print('tc:',tc.shape) # torch.Size([5, 18])
                # print('y:',y.shape) # torch.Size([5, 18, 512])
                dy = self.diffeq(tc, y)
