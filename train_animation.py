...
from training.coaches.coach_animation import Coach
...
@click.command()
@click.option('-i', '--input_folder_source', type=str, help='face id', required=True)
@click.option('-i', '--input_folder_target', type=str, help='expression', required=True)
...
def _main(input_folder_source, input_folder_target, output_folder, start_frame, end_frame, run_name,
    ...
    files_source = make_dataset(input_folder_source)
    files_source = files_source[start_frame:end_frame]

    files_target = make_dataset(input_folder_target)
    files_target = files_target[start_frame:end_frame]
    
    print(f'Number of images: {len(files_source)}')
    image_size = 1024
    print('Aligning images')
    crops_source, orig_images, quads = crop_faces(image_size, files_source, scale,
                                           center_sigma=center_sigma, xy_sigma=xy_sigma, use_fa=use_fa)
    crops_target, _, _ = crop_faces(image_size, files_target, scale,
                                           center_sigma=center_sigma, xy_sigma=xy_sigma, use_fa=use_fa)
    print('Aligning completed')

    ds_source = ImageListDataset(crops_source, transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]))
    ds_target = ImageListDataset(crops_target, transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]))
    coach = Coach(ds_source, ds_target, use_wandb)

    print('start pivot tuning')
    # ws = coach.train()
    w_pivots, cm_pivots, w_animations = coach.train()
    ...
    for i, (coeffs, crop, orig_image, w, cm, w_animation) in tqdm(
            enumerate(zip(inverse_transforms, crops_source, orig_images, w_pivots, cm_pivots, w_animations)), total=len(w_pivots)):
        w = w[None]
        cm = cm[None]
        with torch.no_grad():
            ...
            animation_img = pit_gen.get_final_output(styles=w_animation, camera_matrices=camera_matrices)
            background_mask_img = content_mask * gen_img + (1-content_mask)* crop_tensor
            animation_img = content_mask * animation_img + (1-content_mask)* crop_tensor
            
            gen_img = to_pil_image(gen_img)
            background_mask_img = to_pil_image(background_mask_img)
            ori_img = to_pil_image(ori_img)
            animation_img = to_pil_image(animation_img)
        ...
        save_image(paste_image(coeffs, animation_img, orig_image), output_folder, 'animation_img', i)
